...
import 'package:e_commerce_app/data/models/AddcartresponseDm.dart';
...
class HomeDatasourceImpl implements HomeDatasource {
  ...
  @override
  Future<Either<Failure, AddProductCartResponseDm>> addCart(
      String productId) async {
    try {
      var token = SharedPrefernceUtilis.getData('token');
      var response = await apiManager
          .postData(apiEndpoints: ApiEndpoints.addCartendpoint, body: {
        'productId': productId,
      }, headers: {
        'token': token
      });
      var AddCartResponse = AddProductCartResponseDm.fromJson(response.data);

      if (response.statusCode! >= 200 && response.statusCode! < 300) {
        return Right(AddCartResponse);
      } else {
        return Left(Failure(errorMessage: AddCartResponse.message!));
      }
    } catch (e) {
      return Left(Failure(errorMessage: e.toString()));
    }
